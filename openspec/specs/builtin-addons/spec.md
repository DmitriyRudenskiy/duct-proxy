# Built-in Addons Specification

## Purpose

Define the built-in addons: ModifyHeaders, ModifyBody, Block, and Filter. These addons provide common functionality for modifying HTTP requests/responses and filtering flows.
...
- **WHEN** `Block::url(regex)` is configured
- **THEN** requests matching the URL pattern are marked as blocked (metadata flag set)

#### Scenario: Block by header
- **WHEN** `Block::header(name, value)` is configured
- **THEN** requests with matching header are marked as blocked

#### Scenario: Block by source IP
- **WHEN** `Block::source_ip(ip_range)` is configured
- **THEN** requests from matching IP ranges are marked as blocked
