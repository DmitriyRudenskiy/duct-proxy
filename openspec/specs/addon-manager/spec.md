# AddonManager Specification

## Purpose

Define the `AddonManager` that manages addon registration and sequential dispatch. The manager provides error isolation and configurable error policies.
...
- **WHEN** `dispatch_requestheaders(flow: &mut FlowBase)` is called
